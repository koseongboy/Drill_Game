...
namespace DrillGame.UI
{
    public class UILoader : MonoBehaviour
    {
        ...
        public void ShowAlert( string text )
        {
            ...
            void DataInitializer(GameObject newInstance)
            {
                if (newInstance.TryGetComponent<UI_Alert>(out var uiAlert))
                {
                    uiAlert.UpdateUI(text);
                    newInstance.SetActive(true); // 로드 후 바로 활성화
                }
        ...
        public void ShowUI_ItemDetail(int itemId)
        {
            var uiName = "UI_ItemDetailPopup";
            // 1. 이미 로드되어 있는 경우 (기존 ShowUI 로직 재활용)
            if (loadedUIs.TryGetValue(uiName, out var uiInstance) && uiInstance != null)
            {
                uiInstance.SetActive(true);
                ...
                // 데이터 전달 (동기적)
                if (uiInstance.TryGetComponent<UI_ItemDetailPopup>(out var uiEngineMerger))
                {
                    uiEngineMerger.SetData(itemId);
                }
                return;
            }
            ...
            void DataInitializer(GameObject newInstance)
            {
                if (newInstance.TryGetComponent<UI_ItemDetailPopup>(out var uiEngineMerger))
                {
                    uiEngineMerger.SetData(itemId);
